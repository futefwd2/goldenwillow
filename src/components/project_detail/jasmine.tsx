...
import towerJasmineImg from "../../assets/jasmin_tower/towerjasmine.jpg 1.png";
...
export default function Jasmine() {
  ...
  // Find Jasmine tower in the data
  const jasmineTower = data.find((t) => t.id === 1 || t.name === "JASMINE");

  if (!jasmineTower) {
    return (
      <div className="flex justify-center items-center h-screen bg-[#1e2329] text-white">
        <p>Jasmine Tower data not found.</p>
      </div>
    );
  }
  ...
  return (
    <div className="relative h-screen w-screen overflow-hidden flex justify-center items-center bg-[#1e2329]">
      ...
      <svg
        viewBox="0 0 3000 1983"
        className="w-full h-full select-none"
        preserveAspectRatio="none"
      >
        <image href={towerJasmineImg} x="0" y="0" width="3000" height="1983" />

        {/* Polygons & Tooltips */}
        {jasmineTower.floorsData?.map((floor: any) => {
