...
import towerAsterImg from "../../assets/Aster/toweraster.jpg";
...
export default function Aster() {
  ...
  // Find Aster tower in the data
  const asterTower = data.find((t) => t.id === 9 || t.name === "ASTER");

  if (!asterTower) {
    return (
      <div className="flex justify-center items-center h-screen bg-[#1e2329] text-white">
        <p>Aster Tower data not found.</p>
      </div>
    );
  }

  const viewBox = (asterTower as any).imageSettings?.viewBox || "0 0 3000 1688";
  const imageWidth = (asterTower as any).imageSettings?.width || "3000";
  const imageHeight = (asterTower as any).imageSettings?.height || "1688";
  ...
  return (
    <div className="relative h-screen w-screen overflow-hidden flex justify-center items-center bg-[#1e2329]">
      ...
      >
        <image href={towerAsterImg} x="0" y="0" width={imageWidth} height={imageHeight} preserveAspectRatio="none" />

        {/* Polygons & Tooltips */}
        {asterTower.floorsData?.map((floor: any) => {
          let yCenter = 0;
          let xMax = 0;
              ...
              {hasPolygon ? (
                ...
                <polygon
                  ...
                  onDoubleClick={() => {
                    ...
                    navigate(`/golden_floor/${floorParam}`, { state: { floorName: floor.name, towerId: 9 } });
                  }}
                />
              ) : (
