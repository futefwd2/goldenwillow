...
import towerZeniaImg from "../../assets/Zenia/towerzenia.jpg";
...
export default function Zenia() {
    ...
    // Find Zenia tower in the data
    const zeniaTower = data.find((t) => t.id === 8 || t.name === "ZENIA");

    if (!zeniaTower) {
        return (
            <div className="flex justify-center items-center h-screen bg-[#1e2329] text-white">
                <p>Zenia Tower data not found.</p>
            </div>
        );
    }

    const viewBox = (zeniaTower as any).imageSettings?.viewBox || "0 0 3000 1688";
    const imageWidth = (zeniaTower as any).imageSettings?.width || "3000";
    const imageHeight = (zeniaTower as any).imageSettings?.height || "1688";
    ...
    return (
        <div className="relative h-screen w-screen overflow-hidden flex justify-center items-center bg-[#1e2329]">
            ...
            >
                <image href={towerZeniaImg} x="0" y="0" width={imageWidth} height={imageHeight} preserveAspectRatio="none" />

                {/* Polygons & Rectangles & Tooltips */}
                {zeniaTower.floorsData?.map((floor: any) => {
                    let yCenter = 0;
                    let xMax = 0;
                            ...
                            {hasPolygon ? (
                                ...
                                <polygon
                                    ...
                                    onDoubleClick={() => {
                                        ...
                                        navigate(`/golden_floor/${floorParam}`, { state: { floorName: floor.name, towerId: 8 } });
                                    }}
                                />
                            ) : (
